...
public class ImmediateJobRunnerTest {
	...
	public void testRunExistingJob() {
		...
		when(repoMock.getFunction("BAPI_XBP_JOB_START_IMMEDIATELY")).thenReturn(functionMock);
		
		/* FIN DE MOCK -------------------------------------------------------------------- */

		ImmediateJobRunner jobRunner = new ImmediateJobRunner(repoMock);
		...
		verify(repoMock, times(1)).getFunction("BAPI_XBP_JOB_START_IMMEDIATELY");
	...
	public void testRunNonexistentJob() {
		...
		when(mockRepo.getFunction("BAPI_XBP_JOB_START_IMMEDIATELY")).thenReturn(mockFunction);

		ImmediateJobRunner jobRunner = new ImmediateJobRunner(mockRepo);
		JobRunData jobData = JobData.newJobRunData("SOME_JOB", "mzaragoz", "123456");
		SapBapiret2 runRet = jobRunner.runJob(jobData);
