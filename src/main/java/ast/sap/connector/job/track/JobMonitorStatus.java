...
import ast.sap.connector.job.track.JobMonitor.SapAction;
...
public class JobMonitorStatus {

	private static final Logger LOGGER = LoggerFactory.getLogger(JobMonitorStatus.class);
	...
	/**
	 * Bandera utilizada para testing. Indica si el monitor al dormir debe llamar a
	 * Thread.sleep.
	 */
	boolean doSleep = true;

	/**
	 * Bandera utilizada para testing. Indica si el monitor al reconectarse debe
	 * llamar a {@link Connector} o no.
	 */
	boolean doReconnect = true;

	public JobMonitorStatus(SapRepository sapRepository) {
		this.sapRepository = sapRepository;
	}

	/**
	 * Monitorea un job hasta que el mismo termine. Imprime el log del job a medida
	 * que se ejecuta y obtiene el log completo al finalizar la ejecucion.
	 * 
	 * @param jobData           - Informacion del job a correr y monitorear.
	 * @param printContinuously - [OPCIONAL] true si se desea que el log se imprima
	 *                          continuamente. False en caso contrario. VALOR POR
	 *                          DEFECTO: FALSE.
	 * @return Log del job y su estado.
	 */
	public JobFullStatus monitorJobStatus(final JobTrackData jobData, boolean... printContinuously) {
		...
		while (jobRunning) {
			...
			/*
			 * Si se habilito impresion continua -> el log del job se imprime en a medida
			 * que se monitorea
			 */
			if (pContinuously)
				currLogLine = printLog(jobData, currLogLine);
		...
		final JoblogReadData jobLogReadData = new JoblogReadData(jobData.getJobName(), jobData.getJobId(),
				jobData.getExternalUsername());
		JobLog jobLog = getJobLog(jobLogReadData);
		/*
		 * Si NO se habilito impresion continua -> el log del job se imprime todo junto
		 * cuando la ejecucion finaliza
		 */
		if (!pContinuously)
			jobLog.printLogStdout();

		printStatus(jobStatus);

		printJopsTable(jobStatus);
	...
	private void printStatus(JobStatus jobStatus) {
		String aux = jobStatus.getStatusCode().name();

		if (aux.equals("F")) {
			System.out.println("codError: 0");
			System.out.println("statusJops:" + jobStatus.getStatusCode());
		} else if (aux.equals("A")) {
			System.err.println("codError: 1");
			System.out.println("statusJops:F - Finished");
	...
	private void printJopsTable(JobStatus jobStatus) {
	...
	private void printCodError(JobStatus jobStatus) {
		System.out.println(jobStatus.getReturnStruct().getId());
	}

	/**
	 * Imprime el log del job en standard output.
	 * 
	 * @param jobData     - Informacion del job cuyo log se debe imprimir.
	 * @param currLogLine - Linea actual de lectura del log.
	...
	private int printLog(JobTrackData jobData, int currLogLine) {
		...
		final JoblogReadData jobLogReadData = new JoblogReadData(jobData.getJobName(), jobData.getJobId(),
				jobData.getExternalUsername(), Direction.BEGINNING);
	...
	private JobStatus getJobStatus(final JobTrackData jobData) {
		return doAndReconnect(new SapAction<JobStatus>() {
			...
			public JobStatus act() throws FunctionGetFailException {
				return JobTracker2.build(sapRepository).getStatus(jobData);
			}
	 ...
	 * 
	 * @param action Accion a realizar contra el server sap.
	 * @param errMsg Mensaje de error a mostrar en caso de falla de conexion.
	 * @return Resultado de la accion.
	 * @throws ReconnectFailException En caso que se haya superado la cantidad de
	 *                                intentos de reconexion.
	 * 
	 */
	// TODO : LOS TIPOS DE EXCEPCION CATCHEADOS SON INCORRECTOS. VERIFICAR CUAL ES
	// EL TIPO DE EXCEPCION ANTE ERRORES DE CONEXION
	private <E> E doAndReconnect(SapAction<E> action, String errMsg) throws ReconnectFailException {
		try {
			...
			reconnect();
			// luego de haber reestablecido la conexion, ejecuto recursivamente este mismo
			// metodo
	...
	/**
	 * Intenta reestablecer la sesion con sap. El numero de intentos esta
	 * determinado por {@link Configuration#getReconnectMaxTries()}.
	 * 
	 * @throws ReconnectFailException En caso que se haya superado la cantidad de
	 *                                intentos de reconexion.
	 */
	...
	void reconnect(int tryCount, int maxTries, Exception... lastEx) throws ReconnectFailException {
		...
		try {
			...
			LOGGER.error("Intento de reconexion {} fallo...", tryCount);
			sleep(1000);
